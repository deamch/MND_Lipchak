...
def function(x1, x2, x3):
    y = 0.6 + 4.0 * x1 + 2.8 * x2 + 4.7 * x3 + 3.1 * x1 * x1 + 0.4 * x2 * x2 + 5.4 * x3 * x3 + 5.7 * x1 * x2 + \
        0.1 * x1 * x3 + 8.8 * x2 * x3 + 0.1 * x1 * x2 * x3 + randrange(0, 10) - 5
    return y
...
x1 = [x1_min, x1_min, x1_min, x1_min, x1_max, x1_max, x1_max, x1_max, -1.73 * x1_delt + x01, 1.73 * x1_delt + x01, x01, x01,
      x01, x01, x01]
x2 = [x2_min, x2_min, x2_max, x2_max, x2_min, x2_min, x2_max, x2_max, x02, x02, -1.73 * x2_delt + x02, 1.73 * x2_delt + x02,
      x02, x02, x02]
...
def find_kn(num):
    a = 0
    for j in range(n):
        a += aver_y[j] * list_for_a[j][num - 1] / n
    return a
...
det_2 = [my, find_kn(1), find_kn(2), find_kn(3), find_kn(4), find_kn(5), find_kn(6), find_kn(7),
        find_kn(8), find_kn(9), find_kn(10)]
